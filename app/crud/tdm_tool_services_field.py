...
def get_all_tools(db: Session):
    try:
        return db.query(TDMToolWorkflowField).all()  # ✅ Updated class name
    except SQLAlchemyError as e:
        raise DBReadError(message=f"DB operation failed while reading tools: {str(e)}")

def get_tool_by_id(db: Session, tool_id: str):
    try:
        tool = db.query(TDMToolWorkflowField).filter(TDMToolWorkflowField.tool_id == tool_id).first()  # ✅ Updated
        if not tool:
...
def create_tool(db: Session, tool: TDMToolWorkflowFieldCreate):  # ✅ Updated schema name
    try:
        ...
        db_tool = TDMToolWorkflowField(**tool.model_dump())  # ✅ Updated model
        db.add(db_tool)
        db.commit()
        db.refresh(db_tool)
        return db_tool
...
def update_tool(db: Session, tool_id: str, tool: TDMToolWorkflowFieldUpdate):  # ✅ Updated schema name
    try:
