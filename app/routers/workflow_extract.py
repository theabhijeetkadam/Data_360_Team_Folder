...
def extract_workflow_data(
    ...
    # 2) Determine reserved / unreserved subsets
    reserved_rows   = [s for s in selected_rows if s.is_reserved]
    unreserved_rows = [s for s in selected_rows if not s.is_reserved]
    reserved_count  = len(reserved_rows)
    unreserved_count = len(unreserved_rows)
    ...
    # 5) Insert ALL selected rows (reserved and unreserved) into DB
    #    Validate identifiers ONLY if reserving any rows (your original requirement)
    if reserved_count > 0:
    ...
    # Build params for all selected rows
    ...
    for s in selected_rows:
        ...
        reserved_at = now if is_reserved else None

        params = {
            # If your table requires NOT NULL on these, ensure they’re present for all rows.
            ...
            "source_table": "ui_selected_rows",  # informational label
            "data_key": s.data_key,
    ...
    if params_to_insert:
        ...
        db.execute_many(insert_sql, params_to_insert) if hasattr(db, "execute_many") else db.execute(insert_sql, params_to_insert)
        db.commit()
        logger.info("Extract: rows committed", extra={
            "workflow_id": workflow_id,
            "reserved_count": reserved_count,
            "unreserved_count": unreserved_count,
            "total_inserted": len(params_to_insert),
        })
