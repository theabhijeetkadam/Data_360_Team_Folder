...
# app/routers/mining_workflow_output_criteria.py
...
def create_output_criteria(data: MiningWorkflowOutputCriteriaCreate, db: Session = Depends(get_db)):
    try:
        ...
        # 🔁 Change to 400 (Bad Request) makes more sense for invalid workflow_id/names
        return JSONResponse(status_code=400, content=error_response(400, str(e)))

@router.get("/{workflow_id}", response_model=list[MiningWorkflowOutputCriteriaOut])
def get_by_id(workflow_id: int, db: Session = Depends(get_db)):
    record = crud.get_output_criteria_by_workflow_id(db, workflow_id)
    if not record:
...
def update(output_criteria_id: int, data: MiningWorkflowOutputCriteriaUpdate, db: Session = Depends(get_db)):
    try:
        ...
        # from _validate_workflow_id / _autofetch_ids invalid refs
